...
function createPostHtml(postData, largeFont = false) {
    ...
    if (postData == null) {
        return alert('posted object is null');
    }
    ...
    if (postData.replyTo && postData.replyTo._id) {
        if (!postData.replyTo._id) {
            return alert("Reply to is not populate");
        }
        else if (!postData.replyTo.postedBy._id) {
            return alert("posted by is not populate");
        }
...
function searchUsers(searchTerm) {
    $.get('/api/users', { search: searchTerm }, results => {
        outputSelectableUser(results, $(".resultsContainer"))
    })
}

function outputSelectableUser(results, container) {
    container.html("");
